import { sanitizePrompt, sanitizeOutput } from '../services/security';
...
import { buildExecutionDNA } from '../services/executionDNA';
...
export const handleMentor = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await conversationRef.set({
      messages: FieldValue.arrayUnion(
        ...
        { role: 'assistant', content: safeResponse, timestamp: new Date(), offline: result.offline, provider: result.provider }
      ),
      lastMessageAt: FieldValue.serverTimestamp(),
...
export const handleDailyBriefing = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleWeeklyReport = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
  const input = sanitizePrompt(typeof req.body?.input === 'string' ? req.body.input.slice(0, 1000) : undefined);
  if (!isProjectScopedInput(input)) {
    res.json({
  ...
  // Build Execution DNA and generate Gemini report concurrently
  const [model, executionDNA] = await Promise.all([
    withTimeout(generateGeminiText({
      systemInstruction: systemBase,
      prompt: weeklyReportPrompt(input, memory),
      maxOutputTokens: 850,
    }), 15000),
    buildExecutionDNA(userId),
  ]);
  res.json({
...
export const handleGoalBreakdown = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleReflectionFeedback = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleBudgetDiscipline = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleInterventions = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
  const input = sanitizePrompt(typeof req.body?.input === 'string' ? req.body.input.slice(0, 1000) : undefined);
  if (!isProjectScopedInput(input)) {
...
export const handleSecurityAdvice = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleCortex = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleProofReview = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleReward = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
...
export const handleSubscriptionCheckout = asyncHandler(async (req, res) => {
  const userId = requireUser(req);
  if (!STRIPE_AVAILABLE) {
...
export const handleSubscriptionPortal = asyncHandler(async (req, res) => {
  if (!STRIPE_AVAILABLE) {
    res.status(503).json({ error: { code: 'payments_unavailable', message: getStripeConfigStatus().message } });
    return;
  }
