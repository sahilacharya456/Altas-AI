...
export const retrieveSafeMemory = async (userId: string): Promise<SafeUserMemory> => {
  ...
  return {
    ...
    tasks: tasksSnapshot.docs.map((doc) => compactDoc(doc.data())),
    goals: goalsSnapshot.docs.map((doc) => compactDoc(doc.data())),
    reflections: logsSnapshot.docs.map((doc) => compactDoc(doc.data())),
    ...
    cortexRisk: cortexDoc.exists ? compactDoc(cortexDoc.data() ?? {}) : null,
